...
export function nearestRotateCopyEntity(mouse, lines, circles, arcs, splines = []) {
...
export function buildRotatedCopies(sel, lines, circles, arcs, splines = [], cx, cy, angleDeg, count) {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    for (const e of sel) {
      if (e.kind === 'line') {
        const l = lines[e.idx]
        const p1 = rotatePoint(l.x1, l.y1, cx, cy, rad)
        const p2 = rotatePoint(l.x2, l.y2, cx, cy, rad)
        newLines.push({ x1: p1.x, y1: p1.y, x2: p2.x, y2: p2.y, ...(l.style?{style:l.style}:{}) })
      }
      if (e.kind === 'circle') {
        const c = circles[e.idx]
        const cp = rotatePoint(c.cx, c.cy, cx, cy, rad)
        newCircles.push({ cx: cp.x, cy: cp.y, r: c.r, ...(c.style?{style:c.style}:{}) })
      }
      if (e.kind === 'arc') {
        const a = arcs[e.idx]
        const cp = rotatePoint(a.cx, a.cy, cx, cy, rad)
        newArcs.push({ cx: cp.x, cy: cp.y, r: a.r,
          startAngle: norm2pi(a.startAngle + rad),
          endAngle:   norm2pi(a.endAngle   + rad),
          ...(a.style?{style:a.style}:{}) })
      }
      if (e.kind === 'spline') {
        const sp = splines[e.idx]
        newSplines.push({
          points: sp.points.map(p => rotatePoint(p.x, p.y, cx, cy, rad)),
          closed: sp.closed,
